...
export default function SubscribeModal({
  ...
  pack: Pack | null;
  onCancel: () => void;
  onConfirm: (payload: { email: string; accept: boolean }) => void;
  userWallet?: number;
  onRecharge?: () => void;
}) {
  ...
  const hasFunds = useMemo(() => {
    if (!pack?.price || userWallet == null) return true; // si pas d'info, ne bloque pas
        ...
        <View className="w-full max-w-md rounded-2xl overflow-hidden bg-background-light dark:bg-background-dark border border-outline-100 dark:border-outline-800 shadow-xl">
          ...
          <View className="p-4">
            <View className="flex-row items-start justify-between -mt-10">
              ...
              {pack.price != null ? (
                <View className="px-2 py-1 rounded-md bg-white dark:bg-outline-900 border border-outline-100 dark:border-outline-800 shadow-sm">
                  <Text className="text-xs font-extrabold text-typography-default dark:text-typography-white">{pack.price}€</Text>
                </View>
              ) : null}
            </View>
            ...
            <View className="mt-3 gap-2">
              ...
              <View className="flex-row items-center gap-2">
                <Ionicons name="checkmark-circle" size={16} color="#22c55e" />
                <Text className="text-xs text-typography-default dark:text-typography-white">Valable {pack.durationDays ?? 30} jours</Text>
              </View>
              <View className="flex-row items-center gap-2">
                <Ionicons name="checkmark-circle" size={16} color="#22c55e" />
            ...
            <View className="mt-4 gap-2">
              <Text className="text-xs text-typography-gray">Email de confirmation</Text>
              <TextInput
                value={email}
                ...
                className={`px-3 py-2 rounded-md border ${isEmailValid || !email ? 'border-outline-200 dark:border-outline-800' : 'border-error-500'} bg-white dark:bg-outline-900 text-typography-default dark:text-typography-white`}
              />
              {!isEmailValid && !!email && (
                <Text className="text-xxs text-error-500">Veuillez saisir un email valide.</Text>
              )}
            </View>
            ...
            >
              <View className="w-4 h-4 rounded-[4px] border border-outline-300 dark:border-outline-700 items-center justify-center bg-white dark:bg-outline-900">
                {accept ? <Ionicons name="checkmark" size={12} color="#181c5c" /> : null}
              </View>
              <Text className="text-xs text-typography-default dark:text-typography-white">
                J'accepte les conditions d'utilisation et je confirme mon achat.
              </Text>
            </Pressable>
            ...
            <View className="mt-5 flex-row items-center justify-between gap-2">
                ...
                {!hasFunds && (
                  <>
                    <Text className="text-xxs text-error-500">
                      Solde insuffisant: votre solde est de {userWallet?.toFixed(2)}€, prix du pack {pack.price?.toFixed(2)}€.
                    </Text>
                    {onRecharge && (
              ...
              <Pressable
                disabled={!isEmailValid || !accept || !hasFunds}
                onPress={() => onConfirm({ email, accept })}
                className={`px-4 py-2 rounded-md ${!isEmailValid || !accept || !hasFunds ? 'bg-primary-defaultOrange/60' : 'bg-primary-defaultOrange active:opacity-90'} `}
