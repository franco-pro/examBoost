      ...
      <ScrollView contentContainerStyle={{ padding: 20 }}>
            ...
            <View className="bg-white mt-[7px] p-5 rounded-2xl shadow-sm border border-slate-100 space-y-4">
              ...
              {errorMessage && (
                <View className="flex-row items-center bg-red-50 border border-red-200 p-3 rounded-xl space-x-2">
                  ...
                  <Text className="text-red-700 text-xs flex-1">{errorMessage}</Text>
                </View>
              )}
        ...
        {result && (
          <View className="space-y-6">
            ...
            <View className={`p-6 rounded-2xl border mt-[7px] items-center shadow-sm ${decisionBgClass}`}>
              <Ionicons
                name={isAdmis ? 'checkmark-circle-outline' : 'close-circle-outline'}
                size={56}
